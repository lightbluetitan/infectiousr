...
#' }
#' @export
view_datasets_infectiousR <- function() {
  # Check if the package is loaded
  if (!"infectiousR" %in% .packages()) {
    stop("The 'infectiousR' package must be loaded to view its datasets.")
  }

  # Extract dataset information
  datasets_info <- utils::data(package = "infectiousR")$results

  # Check if there are datasets available
  if (nrow(datasets_info) == 0) {
    message("No datasets are currently available in the 'infectiousR' package.")
    return(character(0))
  }
  ...
  message("Datasets available in the 'infectiousR' package:")
